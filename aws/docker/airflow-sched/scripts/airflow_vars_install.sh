...
aws s3 sync s3://$S3_AIRFLOW_BUCKET/variables/ /usr/local/airflow/variables/ --exact-timestamps --delete --quiet
...
then
    airflow variables --import $VARIABLES_INSTALL_FILE >> $VARIABLES_LOG_FILE
else
