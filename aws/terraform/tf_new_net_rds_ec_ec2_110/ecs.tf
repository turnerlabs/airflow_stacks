...
resource "aws_ecs_task_definition" "airflow_webserver_ecs_task_def" {
  ...
  cpu                       = "500"
  memory                    = "1000"
  execution_role_arn        = "${aws_iam_role.airflow_ecsTaskExecutionRole.arn}"
...
resource "aws_ecs_service" "airflow_webserver_ecs_svc" {
  ...
  depends_on              = [ "aws_alb_listener.airflow_lb_listener", "aws_iam_role.airflow_task", "aws_iam_role_policy.airflow_s3", "aws_iam_role_policy.airflow_logs", "aws_iam_role_policy.airflow_secrets" ]
