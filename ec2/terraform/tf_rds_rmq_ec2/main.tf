...
resource "aws_security_group" "airflow_rabbitmq" {
  depends_on  = ["aws_security_group.airflow_ssh"]
...
resource "aws_instance" "airflow_rabbitmq" {
  depends_on                  = ["aws_security_group.airflow_rabbitmq"]

  instance_type               = "${var.rabbitmq_instance_class}"
  ami                         = "${var.rabbitmq_ami}"
  associate_public_ip_address = false
  key_name                    = "${var.rabbitmq_keypair_name}"
  vpc_security_group_ids      = ["${aws_security_group.airflow_rabbitmq.id}"]
